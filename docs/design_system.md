...
### Glimi 정체성 색 (브랜드 — 보존)
agent 사분면과 대화 상태. **이건 je-empty 톤과 별개로 Glimi 고유 식별색이라 유지한다.** 단, 면을 가득 칠하지 말고 배지/링/점에 절제해 쓴다.
...
- **모션**: `--ease` + `--dur-fast/dur/dur-slow`(0.12/0.2/0.3s). 짧고 차분하게. bounce·spring·과한 scale 금지. hover 는 `translateY(-1px)` 정도.
