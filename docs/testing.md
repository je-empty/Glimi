...
| `community/core/formatting.py` | 포맷팅 테스트 | |
| 씬·튜토리얼·채팅 플로우 | E2E QA | |
...
웹 자체 채팅으로 격리 temp dir 에서 셀프 구동하므로 외부 계정·토큰은 필요 없다.

1. **QA 오너 페르소나에 자기 자신을 넣는다** (`QA_OWNER_NAME` 등) — 에이전트가 이 사람을
   오너로 인식하므로, 본인 이름/호칭이어야 호칭·기억 로직이 실사용 조건으로 검증된다.
   `communities/` 는 gitignore 라 개인정보는 커밋되지 않는다.
...
격리 테스트는 #4 그린 복구 후 게이트에 추가 예정 (#5). E2E QA 는 실 LLM 백엔드 호출(비용·지연)이
있어 CI 부적합 — 로컬 절차로 유지.
