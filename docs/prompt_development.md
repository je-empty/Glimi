...
```
src/core/prompts/        # cross-scene universal (어느 scene 에서든 쓰는 범용)
...
src/scenes/{scene}/      # scene-scoped (해당 scene 만 의미 있음)
...
| 프롬프트 성격 | 위치 |
|---|---|
| agent_type 별 정적 system prompt | `src/core/prompts/en/{persona,mgr,creator}.py` |
| 모든 에이전트 공통 규칙 | `src/core/prompts/en/common.py` |
| 특정 scene 에서만 필요한 fragment | `src/scenes/{scene}/prompts.py` |
| 특정 scene 에서만 필요한 user prompt (1회성) | `src/scenes/{scene}/greeting.py` or `src/scenes/{scene}/events.py` |
| Cross-scene event notification (mgr) | `src/core/prompts/en/mgr_notifications.py` |
| Cross-scene event trigger (persona) | `src/core/prompts/en/persona_events.py` |
| 수동 !명령 | `src/core/prompts/en/commands/{name}.py` |
| 외부 이미지/오디오 모델 | `src/core/prompts/en/external/{name}.py` |
| Supervisor Haiku judge (cross-scene) | `src/core/prompts/en/supervisor_judge.py` |
| Supervisor Haiku judge (scene 전용) | `src/scenes/{scene}/judge_prompts.py` |
...
- **출력 언어는 `[LANGUAGE: X]` 블록**이 강제 (`src/core/prompts/en/common.py` 가 `community.get_language()` 기반 주입). 영어 프롬프트 + `[LANGUAGE: Korean]` → 한국어 응답 정상.
- **한국어 특화 토큰** (ㅇㅇ / ㅋㅋ / 카톡 / 톡방 / 호칭) 은 `src/core/prompts/locale.py` helper 로 주입. 영어 프롬프트가 `simple_ack_examples()` 같은 함수를 호출.
- **구조적으로 다른 문화 block** (예: 한국 존댓말·호칭 progression) 은 locale helper 로 전체 block 반환 (`korean_onboarding_hints()`) 또는 `src/core/prompts/ko/{module}.py` 전체 override.
...
| 프롬프트 **구조·순서** 자체가 다름 | `src/core/prompts/ko/{module}.py` 전체 override |

---

## 3. 모델 dialect — LLM 백엔드 호환성

- `src/core/prompts/model.py` 가 provider 기반 snippet 분기 (`claude` / `ollama` / `vllm` / `llamacpp` / `openai`).
...
# ✅ 권장
from src.core.prompts.model import tool_call_syntax_hint
...
### src/core/* 에서 `import discord` 금지
platform adapter decoupling 원칙. core 는 Discord / Telegram / Web 중립. `src/bot/*` 만 Discord 의존 OK.

### src/core/prompts/ 는 src/bot/ 를 import 하면 안 됨
- 이전에 `src/core/prompts/helpers.formatting_guide()` 가 `src.bot.formatting` 을 lazy import 하던 누수 있었음 (Phase 2-C 에서 `src.core.formatting` 으로 이동 완료).

### scene-local 자료는 scene 폴더에
- `src/scenes/{scene}/` 에 prompts / greeting / judge_prompts / handlers / supervisor / scene 배치
- `src/core/prompts/` 는 **cross-scene universal** 만
...
- `src/core/memory.py` 의 `_validate_fact` 가 저장 전 방어:
...
- 프롬프트 `src/core/prompts/en/common.py` 의 `core_identity_rules()` 에 정리되어 있음.
...
- [ ] 신규 모듈이면 `src/core/prompts/__init__.py` 의 `_get_builder` dispatch 에서 lookup 가능?

---

## 10. 참조 커밋 (학습용)

- `cac7dc8` 프롬프트 빌더 `src/core/prompts/en/` 로 분리
- `8c86ff0` `src.bot.formatting` → `src.core.formatting` 이동 (decoupling)
