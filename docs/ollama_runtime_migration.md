...
> - **재시작**: `src/core/runtime.py`(봇) + `src/llm/ollama.py`(봇+플랫폼 양쪽 import) 수정 → `run.bat` 으로 봇+플랫폼 재시작.
> - **남은 것**: 라이브 디스코드 턴 (오너 직접 대화) 최종 수용 테스트 — 일반대화/a2a/유나 `<tools>` 정확도 확인.
> - **롤백**: `.env` 에서 `GLIMI_LLM_BACKEND` 제거 → 즉시 Claude 복귀.
>
> **후속 (2026-05-30 추가)**:
> - `achievements/judge.py`: 도전과제 판정도 `src.llm` 경유로 이전 → ollama 라우팅. (검증: gemma 판정 정확)
...
- `src/llm/` = 나중에 만든 백엔드 추상화 (claude_cli / anthropic_sdk / ollama). `_select_backend`가 env 기반으로 백엔드 고름.
- 하지만 핵심 대화 엔진 `src/core/runtime.py`는 이 추상화를 **안 거치고** `claude` CLI를 직접 subprocess로 호출. 모델 레지스트리의 ollama 항목도 주석 처리("Phase 2").
- 현재 `GLIMI_LLM_BACKEND=ollama`로 실제 바뀐 건 **메모리 추출(`src/core/memory.py`)뿐.** (memory.py는 `from src.llm import generate` 사용.)
...
### A. 전면 추상화
5곳 전부 `src.llm.generate/stream_lines`로 교체, 직접 subprocess 제거.
...
Claude 경로는 그대로 두고, `provider == ollama`일 때만 `src.llm` 호출하는 분기 추가. 후처리는 공유 헬퍼로 추출.
...
- Claude면 `process.stdout`, ollama면 `src.llm.stream_lines(...)`를 먹임 — 둘 다 라인 단위라 인터페이스 동일.

### 4. 각 호출 지점에 ollama 분기 (5곳)
- 블로킹(1·2·3·5): `provider == ollama` → `src.llm.generate(system, user, model, agent_type, timeout)` 후 기존 `parse_tools_in_output`/`_parse_response`로 흘림.
- 스트리밍(4): `src.llm.stream_lines(...)` → 3번 헬퍼로 처리. **watchdog kill은 프로세스가 없으니** 타입별 타임아웃을 `stream_lines(timeout=...)`로 전달하는 방식으로 대체.
...
4. **`num_predict`** — `src.llm` 호출 시 `max_tokens` 적절히 (persona 짧게, creator 넉넉히). CLI엔 없던 개념.
...
## 영향 범위 / 재시작
- 수정 파일: `src/core/runtime.py` (주), `src/llm/ollama.py` (think 옵션), 선택적 `src/achievements/judge.py`.
- `src/core/*` 수정 → **봇만 재시작** 충분. judge.py 포함 시 봇+플랫폼 둘 다.
...
## 관련 파일
- `src/core/runtime.py` — 대화 엔진 (이 계획의 주 대상)
- `src/llm/__init__.py` — `_select_backend` 백엔드 선택
- `src/llm/ollama.py` — Ollama 백엔드 (urllib, stdlib only)
- `src/llm/claude_cli.py` — Claude CLI 백엔드 (참고: runtime의 재시도/watchdog는 미포함)
