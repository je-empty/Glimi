...
부하는 **전적으로 LLM 추론**이다. 웹 플랫폼·대시보드·채널 어댑터는 순수 파이썬이라 가볍다.
...
Glimi 백엔드(`community.llm` + runtime 스트림 소비기) 경유로 동일 하네스 측정. 항목: blocking/스트리밍
...
### ⚠ num_ctx + 컨텍스트 예산 시스템 — 메모리 기능의 생명줄

Ollama 기본 컨텍스트는 **4096**. Glimi 한 턴 프롬프트는 **시스템 프롬프트(캐릭터+brief
도구레퍼)** 가 persona ≈2700tok / mgr ≈5100tok, 거기에 5-레이어 메모리 주입(≈1200tok) +
최근 대화가 더해진다. 4096 이면 ollama 가 앞부분(시스템·메모리)을 잘라내 → **캐릭터·기억이
통째로 증발** → 반복·온보딩 정지·맥락 상실. (이 프로젝트 강점인 메모리가 죽는 핵심 원인이었음.)

**2단 방어:**
1. `community/llm/ollama.py` 가 요청에 `num_ctx` 를 명시 (기본 **8192**, `GLIMI_OLLAMA_NUM_CTX`).
   하드 floor 2048.
2. `community/core/context_budget.py` 가 **num_ctx 에 맞춰 프롬프트를 동적 조립** — 절대 초과 안 함:
   - num_ctx 가 메모리 풍부도(scale)를 결정: **8192=1.0(기준)**, 4096=0.5(축소), 16384=2.0(풍부)
   - 최근 대화는 토큰 예산 넘으면 오래된 것부터 trim (최신 보존)
   - 시스템 프롬프트만으로 예산 초과 시 로그 경고 (`⚠ [ctx-budget]`)

**실측 floor (시스템 프롬프트가 못 줄이는 바닥):**

| num_ctx | persona | mgr/creator |
|---|---|---|
| 2048 | ✗ 시스템 프롬프트도 안 들어감 | ✗ |
| **4096** | ✓ (메모리 scale≈0.4 축소) | ✗ (시스템 5100tok > 예산) |
| **8192** *(기본/권장)* | ✓ 여유 (scale 1.0) | ✓ 여유 |
| 16384 | ✓ 메모리 2배 (긴 기억) | ✓ |

→ **persona 는 4096 부터, mgr 가 있으면 8192 필요.** 기본 8192 가 모두 커버. KV 캐시가 VRAM 을
더 먹으니(모델당 ~+1GB) 저사양은 8192 유지, 긴 기억 중시면 16384. 검증: 입력 2572tok
프롬프트가 안 잘리고 메모리 정확 회상, num_ctx 별 프롬프트가 항상 윈도우 내에 fit.
