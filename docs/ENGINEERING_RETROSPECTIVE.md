...
Glimi began on **April 4, 2026** as "Project Chaos" — a single commit (`b2c7a08`) of **12,119 lines across 45 files**. Not a scaffold; a fully-formed prototype of a social simulation where AI agents form relationships, talk to each other autonomously, and an owner can quietly watch their friendships unfold. (The first chat surface was a Discord adapter — a bootstrap exit retired once web chat reached parity; see Pivot 1 and §3.) The differentiators were ambitious from day one: agent-to-agent autonomous chat, intimacy scores and relationship dynamics, layered compressed memory, and per-agent emotional state.
...
The original control surface was a **Textual TUI** (`wizard.py` at 1,675 LOC, `dashboard.py` at 1,089), spawning a single Discord-adapter subprocess. One community per process, driven from a terminal. Discord was the first bootstrap chat surface — the cheapest way to validate the transport seam before any in-house chat UI existed.

On **April 22** (`741dbca`), that became a FastAPI + uvicorn daemon: accounts, sessions, community CRUD, and a **subprocess pool running N communities concurrently**. The TUI was demoted to a read-only "attach" client a week later (`0b61f30`), its spawn path deleted so FastAPI became the single source of process control. The web platform later collapsed even the subprocess pool into a per-community `community/platform/web_runtime.py` driven directly by the platform supervisor, and the Discord adapter was retired once web chat reached parity (2026-06-25) — the live and only transport is now web (`GLIMI_TRANSPORT=web`).

**Cost/benefit:** A TUI cannot be a product. The web platform bought multi-tenant hosting (the live `glimi.iruyo.com`), browser accounts, and web-native chat. Notably, the same commit shrank `CLAUDE.md` from 543 to 46 lines, spinning detail into `docs/` — an early signal of a recurring discipline: keep the always-loaded context lean.
...
**A pluggable transport seam, held as an invariant — and cashed out.** From the first `CLAUDE.md`: *the final goal is native web chat; the chat surface is just an adapter behind a transport-neutral seam.* Discord was the first adapter plugged into that seam — a bootstrap exit chosen only because building chat UI is expensive — and the seam was always the asset: `glimi-core/glimi/transport.py` (`Outbox`/`Speaker`) plus the `ChannelAdapter` protocol in `community/core/channel_adapter.py`, with concrete adapters living under `community/adapters/`. The `platform_decoupling_review.md` audit (Apr 22) found 9 clean core files, 15 legitimate adapters, and **7 leaks**, sized the full decoupling at 5–7 days, and — crucially — **chose to defer it**, with the name-based channel model flagged as the XL-cost item. The principle was finally cashed out in Phase 7's web chat: the live web adapter (`community/adapters/web/`) reached parity, the Discord bootstrap adapter was retired (2026-06-25), and any future transport (Telegram, etc.) now plugs into the *same* seam. A multi-month architectural intention that survived delivery pressure.
