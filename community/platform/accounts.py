...
사용법:
    ...
    python -m community.platform.accounts add <username> [--admin] [--password PW]
    python -m community.platform.accounts remove <username>
    python -m community.platform.accounts reset <username> --password PW
    python -m community.platform.accounts grant <username> <community_id>
    python -m community.platform.accounts revoke <username> <community_id>
    python -m community.platform.accounts bootstrap   # admin 계정 생성 (비번은 프롬프트/env/랜덤)
"""
import argparse
import hashlib
import os
...
def _cli() -> None:
    ap = argparse.ArgumentParser(prog="community.platform.accounts")
    ...
    if args.cmd == "list":
        ...
        if not list_accounts():
            print("  (계정 없음 — `python -m community.platform.accounts bootstrap` 로 기본 계정 생성)")
