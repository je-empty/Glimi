...
def api_snapshot(path: str) -> dict:
    def _run():
        ...
        # demo-en = 영문 데모 (README 영문판 스크린샷용) — 동일 처리.
        if snap.get("community_id") in ("demo", "demo-en"):
            try:
                from community.platform.demo_mock import inject as _demo_inject
                snap = _demo_inject(snap)
            except Exception:
                pass  # mock 실패해도 실제 스냅샷은 반환
        _lang = (snap.get("community_meta") or {}).get("language", "ko")
        for c in snap["channels"]:
            c["last_ago"] = monitor.human_ago(c["last_ts"], _lang)
        return snap
...
def api_usage(path: str) -> dict:
    ...
    """
    def _run():
        from community.adapters.kernel_store import kernel_store
...
def api_communities() -> dict:
    ...
    try:
        from community.platform.supervisor import supervisor
