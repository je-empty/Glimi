...
용도 1: JSON → DB (현재 프로젝트의 profiles/*.json을 DB로 이전)
  python -m community.tools.migrate

용도 2: 기존 DB 업그레이드 (이전 형식 DB를 새 스키마로)
  python -m community.tools.migrate --upgrade-db path/to/old/community.db
"""
import json
import os
import sys
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "--upgrade-db":
        if len(sys.argv) < 3:
            print("사용법: python -m community.tools.migrate --upgrade-db path/to/community.db")
            sys.exit(1)
