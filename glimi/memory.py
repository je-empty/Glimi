...
BUDGET_REACTIONS = 300   # 내 메시지에 달린 리액션 알림 블록 (작게 — perceive-only)

# 리액션을 관계 신호로 환산할 때의 intimacy 증분.
# 일반 메시지 L1 배치의 자연 증분(+1, _try_l1_extract)보다 작게 — 라이크는 작은 친밀 신호.
REACTION_INTIMACY_DELTA = 1
# 친밀도로 환산되는 "긍정" 리액션 이모지 (이외엔 perceive 만, 관계 변동 없음).
POSITIVE_REACTION_EMOJI = frozenset({"❤️", "♥️", "💕", "💖", "😍", "🥰", "👍", "🙂", "😊", "🔥", "✨", "👏"})
...
# ────────────────────────────────────────────────────
# Reaction → relational signal
# ────────────────────────────────────────────────────

# 이미 관계 신호로 환산한 리액션 id 집합 (double-count 방지).
# add_reaction 이 멱등(UNIQUE)이라 같은 리액션은 한 번만 INSERT 되지만, 같은 reaction id 로
# record_reaction_signal 이 두 번 불려도 intimacy 가 두 번 오르지 않게 커널 레벨에서도 가드.
_reacted_signal_ids: set = set()
_reacted_signal_lock = threading.Lock()


def record_reaction_signal(agent_id: str, actor_id: str, emoji: str,
                           reaction_id: Optional[int] = None) -> bool:
    """리액션을 작은 관계/친밀 신호로 EXISTING update_intimacy / relationship-delta 파이프라인에 흘림.

    - perceive-only v1: 에이전트는 리액션을 받기만 함. 이 함수는 "오너가 내 메시지에 ❤️" 같은
      긍정 리액션을 작은 intimacy 증분(REACTION_INTIMACY_DELTA)으로 환산.
    - budget-gate: 긍정 이모지(POSITIVE_REACTION_EMOJI)만 관계에 반영. 그 외엔 perceive 만.
    - double-count guard: reaction_id 별로 1회만 (재주입해도 재증분 안 함).
    - 관계 레코드가 없으면 skip (mgr 가 명시 생성한 관계만 존중 — _try_l1_extract 와 동일 규율).

    Returns: 실제로 신호가 반영됐으면 True, (이모지 부적격/중복/관계없음/스토어 미주입) skip 이면 False.
    """
    if _store is None:
        return False
    if emoji not in POSITIVE_REACTION_EMOJI:
        return False
    # double-count guard (reaction id 있을 때만 — 없으면 best-effort 매번 적용 가능하나
    # 안전하게 id 없는 호출은 (actor, agent, emoji) 조합도 1회로 묶는다).
    guard_key = reaction_id if reaction_id is not None else (agent_id, actor_id, emoji)
    with _reacted_signal_lock:
        if guard_key in _reacted_signal_ids:
            return False
        _reacted_signal_ids.add(guard_key)

    try:
        rel = _store.get_relationship(agent_id, actor_id) or _store.get_relationship(actor_id, agent_id)
        if not rel:
            return False
        # agent_a/agent_b 방향 맞추기 (update_intimacy 는 저장된 방향 기준).
        if rel["agent_a"] == agent_id:
            key_a, key_b = agent_id, actor_id
        else:
            key_a, key_b = actor_id, agent_id
        _store.update_intimacy(key_a, key_b, REACTION_INTIMACY_DELTA)
        try:
            _store.add_relationship_delta(
                key_a, key_b, delta_type="intimacy",
                reason=f"reacted {emoji}",
            )
        except Exception as e:
            print(f"[Memory] reaction relationship_delta 저장 실패: {e}")
        return True
    except Exception as e:
        print(f"[Memory] record_reaction_signal 실패: {e}")
        # 가드는 이미 박았지만, 실패 시 재시도 가능하도록 롤백.
        with _reacted_signal_lock:
            _reacted_signal_ids.discard(guard_key)
        return False
