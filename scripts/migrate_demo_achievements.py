...
ACHIEVEMENTS = [
    ...
    ("first_friend_chat", "done",
        {"friend": "박소은", "channel": "dm-박소은", "msgs": 8},
        29, 29),
    ("three_friends", "done",
        {"talked_to": ["박소은", "김민서", "정예린"]},
        27, 26),
    ("group_chat", "done",
        {"channel": "group-친구들", "msgs": 14, "members": ["박소은", "김민서", "정예린"]},
        24, 24),
    ("peek_internal", "done",
        {"channels": ["internal-dm-박소은-정예린"], "first_overheard_at": "박소은↔정예린 대화"},
        22, 22),
    ("agent_auto_chat", "done",
        {"channel": "group-친구들", "trigger": "오너 부재 4시간 → orchestrator group revive"},
        20, 20),
    ("late_night", "done",
        {"channel": "dm-박소은", "at_hour": 2, "msgs": 6},
        18, 18),
    ("chatter", "done",
        {"count": 1200},
        15, 15),
    ("secret_keeper", "done",
        ...
        12, 12),
    ("many_friends", "done",
        {"count": 7, "agents": ["박소은", "김민서", "한서아", "정예린", "이하린", "최수연", "강수진"]},
        10, 10),
    # 진행 중 (unlocked, 미완)
    ("long_relationship", "unlocked",
        {"days_since_first_chat": 30, "need": 90, "agent": "박소은"},
        25, None),
    ("matchmaker", "unlocked",
        {"count": 4, "need": 10, "matches": ["박소은↔정예린", "김민서↔최수연", "이하린↔정예린", "강수진↔최수연"]},
        14, None),
...
NEW_EVENTS = [
    ...
    ("자율대화", ["agent-persona-001", "agent-persona-002"],
     "오케스트레이터가 박소은↔김민서 internal-dm 자동 시작 — 둘이 게임 얘기로 1시간 수다",
    ...
    ("심야대화", ["owner", "agent-persona-001"],
     "새벽 2시 dm-박소은 — 인생 고민 공유 (도전과제: late_night)",
