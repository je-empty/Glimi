...
async def _h_create_room(args: dict, ctx: ToolContext):
    ...
    from community.core.profile import get_user_name
...
async def _h_start_conversation(args: dict, ctx: ToolContext):
    from community.bot.mgr_system import yuna_start_conversation
    names = args.get("names", [])
...
async def _h_stop_conversation(args: dict, ctx: ToolContext):
    from community.bot.conversation_bridge import stop_conversation, list_active_conversations
...
async def _h_invite_owner(args: dict, ctx: ToolContext):
    from community.bot.mgr_system import yuna_invite_owner
...
async def _h_set_topic(args: dict, ctx: ToolContext):
    from community.bot.mgr_system import yuna_set_channel_topic
    s = f"{args['target']} {args['value']}"
...
async def _h_purge_messages(args: dict, ctx: ToolContext):
    from community.bot.mgr_system import yuna_purge_messages
...
async def _h_recover_channel(args: dict, ctx: ToolContext):
    from community.bot.mgr_system import yuna_restore_discord
    await yuna_restore_discord(ctx.channel_obj, args["target"], ctx.guild)
    return {"target": args["target"]}


async def _h_set_emotion(args: dict, ctx: ToolContext):
    from community.bot.mgr_system import yuna_change_emotion
    from community.core.emotion_emoji import register_emoji_for
...
async def _h_update_profile(args: dict, ctx: ToolContext):
    from community.bot.mgr_system import yuna_edit_profile
...
async def _h_update_relationship(args: dict, ctx: ToolContext):
    from community.bot.mgr_system import yuna_edit_relationship
...
async def _h_reset_agent(args: dict, ctx: ToolContext):
    from community.bot.mgr_system import yuna_wipe_agent
...
async def _h_request_dev_task(args: dict, ctx: ToolContext):
    from community.bot.mgr_system import yuna_dev_request
    from community.bot import MGR_ID
...
async def _h_dev_organize(args: dict, ctx: ToolContext):
    ...
    """
    from community.core.dev_agent import DEV_ID, get_request, mark_analyzed
...
async def _h_dev_escalate(args: dict, ctx: ToolContext):
    ...
    """
    from community.core.dev_agent import DEV_ID, get_request, mark_needs_human_review
...
async def _h_create_agent_profile(args: dict, ctx: ToolContext):
    from community.bot.mgr_system import _cmd_profile_create
    try:
        import json as _j
        raw = args.get("args", "")
...
async def _h_delete_agent_profile(args: dict, ctx: ToolContext):
    from community.bot.mgr_system import _cmd_profile_delete
    await _cmd_profile_delete(ctx.channel_obj, args["name"])
    return {"name": args["name"]}


async def _h_set_profile_image(args: dict, ctx: ToolContext):
    from community.bot.mgr_system import _apply_sample_profile_image
    from community.bot.profile_preview import get_recent_preview, clear_preview
...
async def _h_create_agent_with_image(args: dict, ctx: ToolContext):
    ...
    async def _bg():
        ...
        from community.bot.mgr_system import _cmd_profile_create, _forward_action_to_yuna
        from community.bot.core import (
            send_as_agent, send_image_as_agent, update_agent_webhook_profile_image,
        )
...
async def _h_approve_request(args: dict, ctx: ToolContext):
    from community.bot.mgr_system import yuna_approve_action
    s = json.dumps({
...
async def _run_query(name: str, args_str: str, ctx: ToolContext) -> str:
    ...
    from community.bot.mgr_system import execute_yuna_query
...
async def _h_bring_friend(args: dict, ctx: ToolContext):
    ...
    """
    from community.bot.mgr_system import _forward_action_to_yuna
    from community.core.profile import get_user_id, get_user_name
    ...
    try:
        ...
        from community.bot.mgr_system import _forward_action_to_yuna as _fwd
        import json as _json
