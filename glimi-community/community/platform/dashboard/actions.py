...
def _channel_category(name: str) -> str:
    """채널 이름 → 기대 카테고리. community.bot.core._get_category_for_channel 의 사본 (의존성 회피)."""
...
def api_action_run_sync(body: dict, community_id: str) -> dict:
    ...
    if guard:
        return guard
    from community.core.sync import run_sync
...
def api_action_arrange_channels(body: dict, community_id: str) -> dict:
    ...
    if guard:
        return guard
    from community.core.sync import run_arrange
...
def api_action_restore(body: dict, community_id: str) -> dict:
    ...
    if guard:
        return guard
    from community.core.sync import run_restore
