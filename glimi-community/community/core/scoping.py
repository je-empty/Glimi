...
# 기본: 01:00 ~ 07:59 (커뮤니티 로컬 시간) 동안 supervisor 가 페르소나 발화를 트리거하지 않음.
# 환각 방지 + 새벽 자율 대화 폭주 차단. 실제 사용자 활동 시간대 보호.
# 설정: GLIMI_QUIET_HOURS="1-8" (start-end, 24h) | "off" (비활성)
#       GLIMI_TIMEZONE="Asia/Seoul" (IANA — 미설정 시 KST +9 기본)
QUIET_HOUR_START = 1   # 1시부터
QUIET_HOUR_END = 8     # 8시 전까지 (01:00 ~ 07:59)


def _quiet_hours_range() -> tuple[int, int] | None:
    """quiet hours (start, end) — env override 반영. 비활성이면 None."""
    raw = os.environ.get("GLIMI_QUIET_HOURS", "").strip().lower()
    if raw in ("off", "none"):
        return None
...
def _local_now() -> datetime:
    """커뮤니티 로컬 시각 — GLIMI_TIMEZONE (IANA) 우선, 기본 KST."""
    tz_name = os.environ.get("GLIMI_TIMEZONE", "").strip()
    if tz_name:
        try:
...
def is_quiet_hour() -> bool:
    """현재 로컬 시각이 quiet hour 범위 안인지 (기본 01:00 ~ 07:59).

    자정 가로지르는 범위도 처리. supervisor.check() 진입 직후 검사.
    """
    rng = _quiet_hours_range()
    if rng is None:
        return False
    start, end = rng
    h = _local_now().hour
    if start >= end:  # 자정 가로지르는 범위
...
def owner_recent_status(persona_ids) -> str:
    ...
    """
    from community.core.profile import get_user_id, get_user_display_name
