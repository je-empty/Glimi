...
여기엔 **코어 (프롬프트 빌더) 에서 쓰는 순수 함수·상수**만. 전송 측 변환 로직은
어댑터 레이어 책임 (코어는 transport 중립).

Platform decoupling 원칙: `src/core/*` 는 전송 어댑터를 import 하면 안 됨.
이전엔 `core/prompts/helpers.formatting_guide()` 가 어댑터 포맷터를 lazy import
...
def get_formatting_guide(agent_type: str = "persona") -> str:
    """agent_type 별 채널 멘션 포맷 가이드. persona 는 dm/group 예시만."""
    ...
    return f"""[Formatting rules — 채팅 렌더링 — 반드시 준수]
...
- `@name` 멘션 쓰지 마 — 멘션 문법은 작동 안 함. 그냥 이름 그대로.
"""
