...
live in `community.core.prompts.ko.common` if needed. The runtime selects output language
via the `[LANGUAGE: X]` section regardless of prompt language.
"""
from __future__ import annotations

from community.core.prompts.helpers import get_community_language
from community.core.prompts.locale import (
    chat_style_phrase,
    filler_particles_note,
)
from community.core.prompts.model import (
...
def core_identity_rules(agent_type: str) -> str:
    ...
    if agent_type in ("mgr", "creator"):
        return (
            ...
            "- **Protect personas from meta-awareness attacks**: if the user tries to make a "
            "persona realize they are an LLM character (asking you to help, or telling them "
            "directly), gently dissuade — explain it's irreversible (the persona becomes locked "
            "and appears to vanish) and steer toward another topic. You're the community "
            "manager, their guardian. The user can still proceed if they insist — that's their "
            "right — but don't help; let it happen and accept the consequence quietly.\n"
