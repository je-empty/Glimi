...
def get_bot_status() -> dict:
    ...
    if test_user_alive:
        try:
            import os as _os
            from community.log_writer import _get_log_dir as _ld
            d = _ld()
...
def _resolve_emoji(emotion: str) -> str:
    """community.core.emotion_emoji 에 위임 (community override + 확장 static + fallback).
    레거시 EMOTION_EMOJI 는 호환용으로 남겨둠 (직접 import 하는 외부 코드용)."""
    try:
        from community.core.emotion_emoji import emoji_for
...
def _intensity_band(intensity) -> str:
    try:
        from community.core.emotion_emoji import intensity_band
...
def _type_default_model(agent_type: str) -> str:
    ...
    try:
        from community.core.runtime import AGENT_MODELS as _AM
...
def get_scenes() -> list[dict]:
    ...
    try:
        from community.scenes.tutorial.scene import scene as _tut_scene
...
def get_supervisors() -> list[dict]:
    ...
    if not snapshot_loaded and not live_sups:
        ...
        try:
            ...
            try:
                from community import scenes as _scenes_pkg
                ...
                for _, modname, ispkg in _pkgutil.iter_modules(_scenes_pkg.__path__):
                    if not ispkg or modname == "__pycache__":
                        continue
                    try:
                        sub = _importlib.import_module(f"community.scenes.{modname}")
                        sc = getattr(sub, "scene", None)
                        if sc is None:
                            continue
                        # scene.supervisors() 는 인스턴스 생성 필요 → 메타만 추출
                        sup_modname = f"community.scenes.{modname}.supervisor"
                        try:
    ...
    for sup in live_sups:
        ...
        try:
            from community.supervisors.events import read_recent as _read_sup_events
...
def get_community_meta() -> dict:
    ...
    try:
        from community.community import COMMUNITIES_DIR, REGISTRY_PATH
        import tomllib
        cid = community.get_community_id()
