...
class TutorialFlowSupervisor(Supervisor):
    ...
    async def _check_profile_collection(self, channels, guild, phase):
        ...
        from community.scenes.tutorial.judge_prompts import TUTORIAL_PROFILE_COLLECTION_QUESTION
    ...
    async def _check_channel_setup(self, channels, guild):
        ...
        loop = asyncio.get_event_loop()
        from community.scenes.tutorial.judge_prompts import TUTORIAL_CREATOR_ICEBREAK_QUESTION
        judgment = await loop.run_in_executor(None, lambda: _judge_conversation(
