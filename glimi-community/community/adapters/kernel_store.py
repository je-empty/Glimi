...
Community app's ``community.db`` module. The kernel modules (once migrated) call
...
from community.core import profile
from glimi.store import KernelStore


class SqliteKernelStore(KernelStore):
    """:class:`KernelStore` over the app's ``community.db`` (SQLite)."""
...
class ProfileOwnerContext:
    """:class:`~glimi.profiles.OwnerContext` over ``community.core.profile``."""
...
class ProfileProviderAdapter:
    """:class:`~glimi.profiles.ProfileProvider` over ``community.core.profile`` +
    ...
    def system_prompt(self, agent_id: str, include_profile_image_template: bool = False,
        ...
        from community.core.prompts.model import set_active_model, reset_active_model
...
class LogWriterObserver:
    """:class:`~glimi.observability.KernelObserver` over ``community.log_writer``
