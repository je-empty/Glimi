...
실행: python -m community.tui.wizard
"""
import asyncio
import json
import os
...
from community.i18n import t, get_language
from community.tui.components import LoadingOverlay, ConfirmDialog
...
class CreateScreen(Screen):
    ...
    def _run_db_init(self, cid: str):
        ...
        result = subprocess.run(
            [_venv_python(), "-c",
             f"from community import community, db; "
             f"community.set_community('{cid}'); "
             f"db.init_db(); "
             f"from community.tools.migrate import migrate_json_to_db; "
...
class ManageScreen(Screen):
    ...
    def _start_bot_process(self):
        cid = self._cid
        # 기존 프로세스 정리
        subprocess.run(["pkill", "-f", f"GLIMI_COMMUNITY={cid}.*community.discord_bot"], capture_output=True)
...
class LanguageScreen(ModalScreen[str]):
    ...
    def on_mount(self):
        ...
        from community.i18n import get_language, get_agent_language
    ...
    def on_select(self, event: OptionList.OptionSelected):
        ...
        if not code:
            return
        if self._mode == "ui":
            from community.i18n import save_ui_language
...
class DevServerScreen(Screen):
    ...
    def _reset_clean(self):
        ...
        from community import db as _db, log_writer
        _db.init_db()
...
class ExportImportScreen(Screen):
    ...
    def _run_apply_db(self, db_path: str, cid: str, is_old: bool):
        ...
        if is_old:
            ...
            env["GLIMI_COMMUNITY"] = cid
            result = subprocess.run(
                [_venv_python(), "-m", "community.tools.migrate", "--upgrade-db", str(target)],
                capture_output=True, text=True, env=env,
                cwd=str(PROJECT_ROOT),
            )
            output = (result.stdout or "").strip()
...
def main():
    ...
    if isinstance(result, tuple) and result[0] == "dashboard":
        ...
        py = _venv_python()
        os.execvp(py, [py, "-m", "community.tui.dashboard", community_id])
