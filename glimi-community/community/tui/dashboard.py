...
실행:
  python -m community.tui.dashboard [community_id]
"""
import sys
import os
import re as _re
...
# (community.db import 시 community.get_community_id()가 호출됨)
...
from community.core.sync import run_sync, run_restore
from community import log_writer
from community.tui.components import LoadingOverlay, ConfirmDialog, MessageActionDialog, ErrorDialog
...
# 이전: TUI 가 자체적으로 community.discord_bot subprocess 를 띄우고 lifecycle 관리 → 단일 머신
...
class DataCache:
    """Textual dashboard의 데이터 캐시. 실제 조회는 community.core.monitor에 위임.
...
class DashboardScreen(Screen):
    ...
    def on_mount(self):
        ...
        if not _platform_alive():
            log_writer.system(
                f"[TUI] ⚠ Glimi platform 미가동 — `python -m community.platform` 으로 띄워야 봇 가동 가능 ({GLIMI_PLATFORM_URL})"
            )
    ...
    #
    # 이전: TUI 가 직접 community.discord_bot subprocess 띄우고 lifecycle 관리.
    ...
    def _start_bot(self):
        ...
        if not _platform_alive():
            log_writer.system(
                f"[TUI] platform 미가동 — 먼저 'python -m community.platform' 실행 필요 ({GLIMI_PLATFORM_URL})"
            )
            return
    ...
    def _render_usage(self):
        ...
        from community.core.runtime import CLAUDE_AVAILABLE, AGENT_MODELS
        items = []
    ...
    def _do_delete_channel(self, ch_name):
        ...
        from community.core.sync import _get_token
        import time
    ...
    def _do_sync_scan(self):
        ...
        try:
            ...
            from community.core.sync import run_scan
...
def main():
    ...
    if result == "wizard":
        py = _venv_python()
        os.execvp(py, [py, "-m", "community.tui.wizard"])
