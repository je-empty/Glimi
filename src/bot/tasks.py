...
async def _check_owner_profile(guild):
    ...
    if first_time:
        ...
        prompt = (
            ...
            f"  4. 너({p_name})랑 하나(윤하나)는 매니저 · 크리에이터라서 이 진실을 아는 멤버.\n"
            f"     새 친구 만들기·톡방 만들기·관계 정리 도와줌.\n"
...
async def yuna_watcher():
    ...
    notify_prompt = (
        ...
        f"\n"
        f"[비응답 처리 엄격]\n"
        f"보고 안 할 거면 텍스트 자체를 쓰지 마. 다음 패턴 모두 **출력 금지** (지침을 대사로 받아적는 것도 금지):\n"
        f"  ✗ (무시) / *(별다른 거 없음)* / (일상 대화) / (조용히 넘어감)\n"
        f"  ✗ (기준 해당 없음) / (응답 안 함) / (이미 끝났으니 패스)\n"
        f"  ✗ '별다른 ...' '별거 ...' '(...)' 로 시작하는 모든 자기 설명\n"
        f"  ✗ \"비응답\" / \"0글자\" / \"응답 생략\" 같은 **지침 문구 자체** 를 발화로 복붙\n"
        f"→ 보고할 말이 없으면 그냥 공백만 출력. 무슨 말을 왜 안 하는지 설명도 하지 마."
    )
