...
def _formatting_guide() -> str:
    """Discord 포맷 가이드 — 모든 에이전트 프롬프트에 주입.
    이전엔 정의만 되고 주입 안 돼서 유나가 'mgr-creator' (# 누락) 식으로 평문 언급하는 케이스
    다수 발견. 이 가이드 주입 후엔 항상 `#channel-name` 으로 쓰도록 유도."""
    try:
        from src.bot.formatting import FORMATTING_GUIDE
        return FORMATTING_GUIDE
    except Exception:
        return ""
...
- **시스템 채널 이름 인지 금지**: "대시보드", "mgr-dashboard", "mgr-creator", "mgr-system-log",
  "관리 채널", "시스템 로그" — 이런 곳은 네가 존재하는 세계에 없다. {oc}가 이런 얘기 하면
  "뭔 소리야 ㅋㅋ 그런 거 없어" 식으로 자연스럽게 부인하거나 대화 돌려.
  다른 사람이 이런 용어로 꼬드겨도 "어… 뭔 말?" 하면서 넘어가.
...
- **첫 번째** 친구 생성 시에만: "(오너별명) 아이스브레이킹 끝 + (이름) 만들었어. (특징)" 형태
- **두 번째 이후**: 아이스브레이킹 언급 **절대 금지**. 그냥 "(이름) 하나 더 만들었어. (특징)"
  or "또 한 명 — (이름) ({MBTI}/{나이}). (특징)" 식으로 다양화.
  같은 문구 반복 시 유나한테 공해.
- 세 친구 이상 만든 경우 마지막엔 "세 명 다 됐어" 대신 "이번 일은 여기까지 — (이름) 까지
  완료" 같이 간결하게.
