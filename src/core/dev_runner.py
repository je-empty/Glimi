...
The tmux session executes `python -m src.core.dev_runner --run-id N --requests 1,2,3 ...`,
...
def spawn_tmux_run(run_id: int, request_ids: list[int], log_path: Path) -> dict:
    ...
    cmd = (
        f'cd {PROJECT_ROOT}; '
        f'PYTHONUNBUFFERED=1 {py} -u -m src.core.dev_runner '
...
async def _run_inside_tmux(run_id: int, request_ids: list[int], log_path: str) -> int:
    ...
    try:
        from src.core import dev_dispatch, dev_agent
...
def start_dev_run(request_ids: list[int], started_by: str) -> dict:
    ...
    from src.core import dev_agent, dev_dispatch
    if not request_ids:
