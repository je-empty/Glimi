...
| 기능 | 한 줄 |
|---|---|
| **멀티 에이전트 런타임** | 에이전트별 모델 오버라이드를 DB 에 저장 — Claude·Ollama·Grok 이 한 fleet 에, 재시작 없이 스왑. |
| **도구 프로토콜** | `<tools>` 인라인 XML — 권한·타입·env 게이팅을 갖춘 선언적 `ToolSpec` 레지스트리. |
| **레이어드 영속 메모리 (L0–L5)** | L0 원본 → L5 고정까지 6단, 시간 기반 fact supersession + 응답 경로 밖 비동기 Haiku 추출. |
| **자율 A2A 대화** | 에이전트끼리 턴 제한·closure 감지가 있는 1:1·그룹 대화를 직접 시작. |
| **Proactive supervisor 레이어** | 입력 없이 도는 유일한 레이어 — 새 페어를 열고, 멈춘 대화를 깨우고, 정체된 씬을 진행시킨다. |
| **라이브 관찰성 대시보드** (`glimi[dashboard]`, 읽기 전용) | Cytoscape.js 그래프, L0–L5 메모리 인스펙터, 채널 뷰어, 도구 호출 타임라인, LLM 비용 카드. |
| **평가 하네스** | LLM-judge 를 곁들인 골든셋 능력 eval + 품질 하락 시 CI 를 떨구는 백엔드 태깅 회귀 게이트. |
| **세대형 EDD QA** | 오너 에이전트가 앱 전체를 구동, **git-SHA 세대**마다 0–100 으로 채점 — flagship 차별점 ([아래 상세](#edd--eval-driven-development-커밋마다-추적되는-품질-)). |
| **비용·지연 정산** | 모든 LLM·도구 호출을 한 choke-point 에서 계측 — 설계상 정직 (로컬/echo 는 $0, *est.* 표시, 실제 지출만 달러). |
| **사람 개입 게이트** (Workspace) | 중대한 액션 둘레의 `승인 / 수정 / 거부` 정책; 절대 멈추지 않음(비대화형은 자동 승인). |
| **자가 치유** (실험적, 기본 비활성) | 에이전트가 수정 요청을 올리면 Opus subprocess 가 소스를 패치하고 요약을 주입한 채 재시작. |

전체 기능 상세 → [Internals](docs/internals.ko.md).

### 런타임 내부

응답 한 번이 **8 레이어**를 거친다 — pre-LLM 5개(프롬프트, 도구, 메모리, 채널, guard), post-LLM 2개(A2A 루프, 자가치유), 그리고 주기적으로 도는 supervisor 티어. 메모리는 시간 기반 fact supersession 을 갖춘 6단 스택(L0 원본 → L5 고정)이고, 상태는 프롬프트 밖에 살기에 모델 스왑과 프로필 수정에도 살아남는다.

런타임 파이프라인 다이어그램, 레이어별 상세, 메모리 아키텍처 다이어그램, 하드닝 규칙, 모델 스왑 보장은 → [**내부 구조 문서**](docs/internals.ko.md).

### Elastic Memory — 어떤 컨텍스트 윈도우에도 맞는 메모리

로컬 모델은 윈도우가 작고(Ollama 4096), 전체 Glimi 프롬프트 — 캐릭터 시스템 + L0–L5 메모리 + 대화 히스토리 — 는 종종 그걸 넘긴다. `Elastic Memory`(`glimi/context_budget.py`)가 메모리를 설정된 `num_ctx`(기준 8192; 4096 이면 회상을 줄이고 16384 면 두 배)에 맞춰 크기를 잡고, 가장 오래된 대화부터 잘라내 같은 에이전트가 어느 윈도우에서든 성격을 잃지 않고 돈다. 백엔드 무관·하드웨어 인지 — `community/core/system_specs.py` 가 RAM/VRAM 을 읽어 Low 4096 / Mid 8192 / High 16384 티어를 품질 슬라이더처럼 제안한다.

→ 상세: [Internals](docs/internals.ko.md#elastic-memory--컨텍스트-윈도우에-맞춘-메모리).
...
`Glimi` 가 기본값을 자동 배선한다 — 인메모리 `KernelStore`, 단순 `ProfileProvider`/`OwnerContext`, `NullObserver`, 그리고 고른 백엔드. 자체 DB 로 갈아끼우려면 `KernelStore`(필요 시 profile/owner/observer seam 포함)를 구현해 `glimi.runtime.set_store(...)` 로 주입한다. SQLite + 웹 트랜스포트 완성 예시는 `community/adapters/kernel_store.py` 에 있다.

→ 상세: [Internals](docs/internals.ko.md#라이브러리-임베딩-kernelstore-di).

### 웹 대시보드 + 모델 역할

Core 대시보드는 모든 에이전트에 대한 **읽기 전용** 관찰성이다 — Cytoscape.js 그래프, 메모리 인스펙터(L0–L5), 채널 뷰어, 도구 호출 타임라인, 에이전트별 모델 배지. 기본 config 는 역할을 모델별로 쪼갠다(메모리/judge/응답은 Haiku, 추론은 Sonnet, 일회성/자가치유는 Opus) — Sonnet 단일 대비 대략 **10배 저렴**.

대시보드 패널 상세(스크린샷 포함)와 전체 모델 역할 표는 → [**내부 구조 문서**](docs/internals.ko.md).

### 완전 로컬 모드 (Claude 의존성 0)

`GLIMI_LLM_BACKEND=ollama` 면 모든 LLM 호출(페르소나, 매니저 도구, 메모리 추출, supervisor 체크, 도전과제 judge)이 로컬 Ollama 로 라우팅된다 — Anthropic 키 불필요. `GLIMI_LOCAL_TIER` 로 티어를 고른다(`run.sh --local-models`).
...
12 GB GPU 는 2모델 split 을 못 담는다 — `quality`(26b 단일)를 쓴다. 표와 셋업은 **[`docs/local_models.md`](docs/local_models.md)** 참조.
...
Community 는 자체 채팅을 내장한다 — 캐릭터별 사이드바, 묶음 메시지 행(grouped rows), 답글, 반응, 스레드를 갖춘 익숙한 채팅 레이아웃에 라이트/다크 테마, 모바일까지 된다. 대시보드에서 읽던 그 방이 곧 타이핑하는 방이다. 연결 그래프와 채팅은 한 저장소의 두 화면이라, 그래프의 선을 클릭하면 그 대화로 바로 들어간다.
...
웹 챗은 어댑터 하나일 뿐 — 채팅은 Core 안의 플랫폼 중립 outbox/inbox 심(seam, `Outbox`/`ChannelAdapter`)을 거쳐 WebSocket 으로 오가서, 로드맵의 Telegram 등 다른 어댑터가 같은 자리에 붙는다. 토큰도 봇 설정도 필요 없다.
...
Community 는 Glimi Core 위에 **웹 우선** 설계로 올라간다 — FastAPI + WebSocket 플랫폼이 SQLite `community.db` 를 통해 Core(런타임 · 메모리 · supervisor)와 대화하고, **웹 챗이 라이브 트랜스포트**다 — Core 의 플랫폼 중립 `ChannelAdapter` 심을 거쳐 닿기에, 새 트랜스포트가 붙어도 Core 는 채팅 SDK 를 import 할 일이 없다. 채널은 네 종류다 — `dm-{이름}`(매니저 `dm-agent-mgr-001` 포함), `group-{이름들}`, 그리고 오너 읽기 전용인 `internal-dm-{A}-{B}` / `internal-group-{이름들}` — 여기에 런타임 도구 호출 로그용 `logs/system.log` 파일이 더해진다.

엔진 플로우차트와 전체 채널 구조 표는 → [**내부 구조 문서**](docs/internals.ko.md).
...
Glimi Workspace 는 혼자 일해도 팀처럼 동작한다. Coordinator 가 목표에 맞는 전문가 역할 세트를 제안한다(예: researcher / builder / critic, 또는 여행이라면 trail-scout / logistics-planner / gear-guide — 고정된 셋이 아니라 목표마다 생성된다). 프로젝트 맥락은 한 번만 정리하면 된다. 각 에이전트가 이 정보를 공유하므로 새 세션마다 다시 설명하지 않는다. Haiku, Sonnet, 클라우드, 로컬 환경이 달라도 같은 팀이 이어진다. 일하는 도구가 아니라 맥락을 함께 유지하는 상주 인력처럼 동작한다.

Workspace 와 Community 는 같은 Core 위의 서로 다른 앱이다. Workspace 는 일하는 팀, Community 는 기억하는 친구다. Core 는 모놀리식 구조가 아니다. Workspace 는 `glimi` 패키지만 import 한다(Community 코드 없음).
...
```
> 실제 백엔드(Claude/로컬)에서는 `--serve` 가 **작업 라운드를 먼저** 돌린다 — `:8800` 이 열리기까지 몇 분 걸릴 수 있다. 멈춘 게 아니라 콘솔에서 진행 중이다. (`--demo` 는 즉시 서빙.)
...
**AGPL-3.0-or-later** — 강한 카피레프트 라이선스다. Glimi 를 사용·연구·수정·공유할 수 있다. **배포하거나 네트워크로 제공하는 파생물은 AGPL 하에 오픈 상태를 유지하고 저작자 표기를 지켜야 한다.** 독점·판매 버전은 불가하다. 기여는 같은 라이선스로 받고, 저작권은 저자가 보유하며 별도 상업 라이선스를 낼 수 있다. MongoDB, Grafana, Mastodon 과 비슷하다 — 열린 사용과 함께 키우는 성장.
