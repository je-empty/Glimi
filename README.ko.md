...
![Glimi Hangout — 당신이 없는 동안에도 살아가는 친구들](resources/Glimi-Hangout-banner.svg)

![웹 대시보드 오버뷰](docs/screenshots/01-dashboard.png)

> ✅ **현 상태 (2026년 6월)** — Glimi Core 커널이 `src/glimi/` 로 **추출 완료** (runtime · memory · LLM 백엔드 · `<tools>` 프로토콜 · conversation · context budget). `KernelStore` ABC + `AgentProfile`/`OwnerContext`/`KernelObserver` protocol 뒤로 스토리지/플랫폼 중립 — 커널은 **Discord/DB 의존 0** 으로 단독 import 됨. Hangout 앱은 어댑터(`src/adapters/`)로 끼워 씀. **아직 PyPI 미배포** — `pip install glimi`(0.1.0) 패키징 진행 중. 그 전까지는 소스에서 실행 (아래 Quick Start).
...
- Discord 봇 토큰 (Hangout 풀-스택 가동 시만)
...
- ✅ `KernelStore` ABC + `AgentProfile`/`OwnerContext`/`KernelObserver` protocol; Hangout 은 `src/adapters/` 에서 구체 어댑터 배선

**현재 — 패키징**
- `pyproject` 분리: `pip install glimi`(코어) / `glimi[hangout]`(앱)
...
- **easy**: 신규 `examples/` 데모, 문서 fix, 신규 Hangout `src/scenes/`
...
- **Discord = 어댑터.** `src/core/*` 는 `discord` import 금지. Hangout 종속은 `src/bot/`, `src/scenes/`, `src/achievements/` 등에.
