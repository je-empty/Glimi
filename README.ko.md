...
캐릭터 두 줄이면 무리가 서는 건, 그 아래에서 엔진이 나머지를 다 떠안기 때문이다 — 이 엔진이 **Glimi Core** 다. 기억은 프롬프트가 아니라 저장소(기본 SQLite)에 쌓인다. 그래서 재시작하거나 캐릭터의 모델을 Haiku 에서 로컬 Llama 로 갈아 끼워도 관계·사실·고정 기억이 그대로 따라온다. 기억은 설정한 컨텍스트 윈도우 타깃(`num_ctx`)에 맞춰 잘라 넣기 때문에, 4GB 노트북에서도 24GB 워크스테이션에서도 성격이 잘려 나가지 않는다. 모델은 캐릭터마다 클라우드(Claude)와 로컬(Ollama)을 섞어 써도 되고(Grok CLI 도 지원), 로컬로만 돌리면 비용은 0 이다.
...
이 Core 위에 앱을 올린다. 플래그십은 **Glimi Community** — 내장 웹 UI(또는 디스코드)에서 대화하는 'AI 친구들' 무리로, 자기들 채널에서 떠들고, 비밀을 지키고, 당신이 없을 땐 당신 얘기도 하고, 그걸 다 기억한다. 역할을 나눈 작업용 **Glimi Workspace**(Coordinator 가 Researcher·Builder·Critic 에게 일을 배분 — 실시간 라이브 데모 포함)도, `examples/` 의 라이브러리 스타터들도 모두 같은 Core 위에 선다.
...
**1. 컨텍스트 윈도우에 맞추는 메모리 (Elastic Memory).** Glimi 는 메모리 주입을 설정된 컨텍스트 윈도우 타깃(`num_ctx`)에 맞춰 크기를 조절하고, 토큰 추정 예산으로 잘라내 프롬프트가 윈도우 안에 머물게 한다(완벽 보장이 아니라 추정치 + 안전 마진 기반의 best-effort). 그래서 같은 에이전트가 4GB 노트북에서도 24GB 워크스테이션에서도 성격이 조용히 잘려나가는 일 없이 돈다. 에이전트 프레임워크들도 히스토리를 윈도우에 맞게 잘라낼 수는 있지만(CrewAI·Letta·OpenAI Agents SDK·AutoGen·LangGraph 가 각자 어떤 형태로든 한다), Glimi 처럼 메모리 버짓을 **컨텍스트 윈도우 타깃에 맞춰 잡고 거기 맞게 잘라내는** 곳은 없다. 로컬 런타임들도 안 한다: Ollama 자체의 "VRAM 에 맞춰 컨텍스트 자동 조절" 요청은 2025년부터 미해결 이슈로 열려 있다.
...
- **설계된, 영속적인 인구.** 각 에이전트의 페르소나와 모델을 정의하고, 클라우드(Claude)와 로컬(Ollama)을 한 fleet 에 섞는다. 상태가 프롬프트가 아니라 스토리지에 살기 때문에, 모델을 갈아끼워도 에이전트는 모든 기억과 관계를 유지한다. 에이전트별 모델 선택 자체는 흔하다(Letta·CrewAI·AutoGen 다 됨). 드문 건 그걸 스왑에도 살아남는 영속 상태와 묶은 점이다.
- **스스로 움직이는 에이전트.** proactive supervisor 가 타이머로 돌며 새 에이전트-간 대화를 열고, 멈춘 채널을 되살리고, 씬을 진행시킨다. 그래서 인구가 당신 메시지 사이에도 계속 살아간다. 대부분의 프레임워크는 순수 reactive 다. 자율성을 제대로 구현한 프로젝트(Stanford 의 마을, AI Town)는 연구 코드거나 게임 스택이지, 위에 빌드할 수 있는 라이브러리가 아니다.
...
- **인구 대시보드 내장.** 실시간 웹 UI 가 엔진과 함께 온다: 에이전트 관계 그래프, 에이전트별 메모리 인스펙터(L0–L5), 라이브 채널 뷰어, 에이전트별 모델 조회. 무료 로컬 에이전트 대시보드는 이미 있지만(Letta ADE, Hermes HUD) 한 번에 한 어시스턴트를 들여다본다. Glimi 는 인구 전체의 *관계*를 중심으로 본다.

나머지는 솔직하게: Glimi 는 알파(0.1.0, 아직 PyPI 미배포)고, 거의 모든 개별 기능에는 더 강한 선두주자가 있다 — 순수 메모리 페이징은 Letta, 자율 마을 경험은 AI Town, 캐릭터 도구는 SillyTavern, 시간 그래프는 Zep. Glimi 의 승부수는 개별 항목 하나가 아니라 그 조합이다.
...
Workspace 와 Community 는 *같은* Core 위에 의도적으로 다르게 지은 두 앱이다 — 한쪽은 상주 작업팀, 다른 쪽은 당신을 기억하는 친구들. 그게 핵심이다: 한 커널 위의 뚜렷이 다른 둘째 앱이야말로 Core 가 모놀리식이 아니라 재사용 가능하다는 증거다. Workspace 는 `glimi` 패키지만 import 한다(디스코드 0, Community 코드 0).

팀은 라운드로빈으로 한 방에서 돌지 않고 실제 팀처럼 상호작용한다: 오너가 Coordinator 에게 DM 을 보내면, Coordinator 가 각 전문가에게 각도를 배분하고, 전문가들이 에이전트-투-에이전트 채널에서 **서로** 토론한 뒤, 전체가 그룹 라운드로 수렴하면 Coordinator 가 결과를 전달한다. 이 상호작용들이 작업 관계로 기록되고, 그게 Community 를 그리는 **그** 연결 그래프의 엣지가 된다 — 그래서 작업팀이 실제 상호작용 망으로 나타나고, 멤버마다 자기 레이어드 메모리(L0–L5)를 가진다.
...
Coordinator 가 중대한 단 하나의 액션 — 최종 결과물 전달 — 을 커밋하기 전에, Workspace 는 그걸 **승인 게이트**로 보낼 수 있다: 오너가 승인/수정/거부하고, 거부 시 결정적 폴백이 들어가며, 결정 기록이 `mgr-approvals`(대시보드에서 확인 가능)에 남는다. 정책은 설정값(`--approve auto|final|off`)이고 절대 멈추지 않는다 — 비대화형 실행(CI·파이프·데모)은 자동 승인된다. 평가자가 찾는 바로 그 HITL 심이다: 중요한 액션에 체크포인트, 사후 관찰 가능.

---

## EDD — eval-driven development (커밋마다 추적되는 품질) ⭐

멀티 에이전트 제품은 *증명*하기 어렵다 — "이제 친구들이 더 진짜 같아졌어"는 느낌이지 숫자가 아니다. Glimi 의 답이 **EDD(eval-driven development)** 다: 자율 **오너 에이전트**(스크립트가 아니라 페르소나)가 실제 앱을 온보딩부터 핵심 저니까지 끝까지 구동하고, 그 세션을 **가중 차원**으로 채점해 단일 **0–100 종합 점수**를 내고, 모든 런을 **git-SHA 앵커 "세대"** 로 레포에 커밋한다. 그래서 `git log` 이 곧 측정된 품질 타임라인이 되고, 커밋마다 제품 품질에 준 영향이 눈에 보인다. 프레임워크는 **`glimi.edd`** — 도메인 중립이고 `glimi` 커널의 일부이며 Community·Workspace **양쪽이 상속**한다(각자 자기 차원 + 오너 에이전트만 구현).

**세대 채점 방식** — 각 차원은 0–10 점 + 가중치, 종합은 가중평균을 0–100 으로 정규화. `critical` 차원은 make-or-break — 하나라도 실패하면 종합 점수와 무관하게 런 전체가 FAIL(높은 대화 점수가 망가진 핵심 저니를 덮을 수 없다). LLM-judge 차원은 오프라인 `echo` 백엔드나 judge 부재 시 **skip**(종합에서 제외, 절대 가짜 점수 X)이라 무료 셀프테스트가 점수를 부풀릴 수 없다. Community 의 6차원:
...
아래는 **이 레포에 실제로 커밋된 세대들**(`tests/e2e/qa_generations/*.json`) — 실제 `claude_cli` 런을 judge 가 채점한 결과로, 각각 돈 시점의 git SHA 가 박혀 있다. N 은 작다(시스템이 아직 새것이라). 핵심은 긴 이력이 아니라 **세대를 거듭하며 데이터가 쌓이는 방법론**이다. 정직하게 읽으면 이미 이야기가 보인다:
...
한 줄 피칭: **제품 품질을 git 추적되는 1급 메트릭으로 계측 — 회귀와 미완 작업까지 포함해 모든 커밋의 영향이 측정·가시화된다.** 아래 대시보드와 PDF 가 그 타임라인을 한눈에 읽는 방법이다.
...
**어댑터(채택자)를 위한 재사용.** `glimi.edd` 는 도메인 중립이고 `glimi` 휠에 포함된다 — 본인 차원 + 오너 에이전트 드라이버만 가져오면 종합 채점, git-앵커 세대 스토어(SQLite + 커밋 JSON), HTML/PDF 리포트가 공짜로 따라온다:
...
Community 는 이 위에 6차원을 구현하고, Glimi Workspace 는 같은 `glimi.edd` 코어를 산출물/위임/A2A 차원으로 채택한다 — 하나의 EDD 프레임워크, 두 앱. 전체 설계: [`docs/qa_system.md`](docs/qa_system.md).

---

## Examples

Community 의 소셜 sim 스캐폴딩 없이 Glimi Core 를 직접 보여주는, 실제로 돌아가는 스타터들. `echo` 백엔드로 의존성 0·API 키 없이 바로 실행되고, 실제 모델로 바꾸면 진짜 협업이 펼쳐진다.
...
- ✅ `KernelStore` ABC + `AgentProfile`/`OwnerContext`/`KernelObserver` protocol; Community 은 `community/adapters/` 에서 구체 어댑터 배선
...
- 레이어드 메모리(L0–L5)는 *컨텍스트 안*에서 동작하지만, 오래 살아온 에이전트는 결국 어떤 윈도우보다도 기억이 커진다. 계획: **에이전트마다 자기 RAG 코퍼스**를 검증된 retrieval 코어 위에 둔다 — 누적된 히스토리·지식을 임베딩·인덱싱하고, 매 턴 프롬프트에 다 싣는 대신 **관련된 것만 검색해 끌어온다**. 기억이 '쓰면 닳는 예산'에서 '질의하는 저장소'가 된다.
- **예상 효과**: 히스토리가 커져도 안 무너지는 회상(검색은 `O(top-k)`, `O(history)` 아님) · 에이전트별로 들여다볼 수 있는 지식 베이스 · 요약 드리프트 대신 *출처 있는* 정확한 회상.
- **레이턴시를 마찰이 아니라 '캐릭터'로**: 검색은 인메모리 기억엔 없는 지연을 만든다. 에이전트가 **온로드 상태에서 스킬/툴로** RAG 를 호출하고 그 기다림을 *캐릭터로* 가린다 — *"잠시만…", "그게 뭐였더라, 기억 더듬는 중…"* — 그래서 검색 지연이 스피너가 아니라 **사람이 기억을 떠올리는 한 박자**로 읽힌다. 랙처럼 느껴질 지연이 오히려 사실감이 된다.
...
가장 영향력 큰 첫 작업: Ollama 기반 로컬 LLM 백엔드를 구현하고 Gemma 4 vs Qwen 3.5 를 3가지 모델 역할(페르소나 chat · supervisor judge · 메모리 추출 JSON)에서 벤치마크. 왜냐면 현재 Glimi 는 Anthropic API 에 크게 기대고 있어, 모델 벤더 중립이라는 약속이 아직 증명되어야 하기 때문. 상세 spec: [`START_HERE.html` §5](START_HERE.html#first-task).
...
- **easy**: 신규 `examples/` 데모, 문서 fix, 신규 Community `community/scenes/`
...
**AGPL-3.0-or-later** — 강한 카피레프트. 누구나 자유롭게 사용·연구·수정·공유할 수 있다. 대신 **배포하거나 네트워크 서비스로 제공하는 파생물은 반드시 AGPL 로 소스를 공개하고 이 프로젝트 저작자 표기를 유지해야 한다** — 닫아서 독점 제품으로는 못 만든다. 기여는 같은 라이선스로 환영하며, 저작권은 저자가 보유해 별도의 상업 라이선스를 줄 수 있다. MongoDB·Grafana·Mastodon 과 같은 "열린 채로 + 컨트리뷰터와 함께 성장 + 독점 free-riding 방지" 노선이다.
