...
> **현황 (2026-06)** — Core 커널은 최상위 `glimi/` 패키지(Discord/DB 의존 0, 단독 import)이고, **두 앱 모두 의존성 주입(DI)으로 그 위에서 돈다**: 각자 자기 `KernelStore`/profile/observer 어댑터를 중립 커널에 주입(Community 는 `glimi-community/community/adapters/`, Workspace 는 `glimi` 패키지만으로). 대시보드 UI 는 `glimi.dashboard` 안의 단일 정본 셸을 셋(커널 데모·Community·Workspace) 다 렌더한다. 아직 PyPI 전이라 소스에서 설치 (`pip install -e "./glimi-core[dashboard]"`). 이제 캐릭터와 얘기하는 기본 경로는 내장 **웹 챗**(라이트/다크, 답글, 반응, 스레드, 모바일)이라 **디스코드는 선택사항** — 예정된 여러 어댑터 중 하나. 최근 추가: **평가 하네스**(골든셋 + LLM-as-judge + 회귀 게이트), **세대형 EDD QA 시스템**(`glimi.edd` — 자율 오너 에이전트가 앱 전체를 구동하고 가중 차원으로 채점해 0–100 품질 점수를 commit-over-commit 추적, `/admin/qa` 대시보드 + PDF 리포트; **[아래 실측 세대 데이터](#edd--eval-driven-development-커밋마다-추적되는-품질-)**), **도구 호출 + 비용/지연 관찰성**, Workspace 의 **사람 개입 승인 게이트(HITL)**.
...
- 5레이어 메모리는 *컨텍스트 안*에서 동작하지만, 오래 살아온 에이전트는 결국 어떤 윈도우보다도 기억이 커진다. 계획: **에이전트마다 자기 RAG 코퍼스**를 검증된 retrieval 코어 위에 둔다 — 누적된 히스토리·지식을 임베딩·인덱싱하고, 매 턴 프롬프트에 다 싣는 대신 **관련된 것만 검색해 끌어온다**. 기억이 '쓰면 닳는 예산'에서 '질의하는 저장소'가 된다.
