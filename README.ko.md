...
| **세대형 EDD QA** | 골든셋 eval 의 통합(integration) 짝: 자율 **오너 에이전트**(내 페르소나)가 앱을 온보딩부터 핵심 저니까지 구동하고, 세션을 가중 차원(Community: 온보딩·친구생성·대화품질·환각없음·누수없음·응답성)으로 채점해 **0–100 품질 점수**. 각 런은 **git-SHA 앵커 "세대"** 로 SQLite + 커밋 JSON 에 남아 commit-over-commit 추적 — `git log` 가 곧 측정된 품질 타임라인(*eval-driven development* flywheel). [`docs/qa_system.md`](docs/qa_system.md) |
