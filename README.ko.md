...
Glimi 는 성격·기억·관계를 가진 AI 캐릭터 무리를 만드는 파이썬 라이브러리다. 캐릭터마다 **페르소나** 와 **모델** 만 정하면 된다. 캐릭터들은 서로, 또 당신과 대화한다. supervisor 가 채널을 주기적으로 관리해 끊긴 대화를 잇고, 자리를 비워도 대화 기록은 남는다.
...
두 줄 정의로 캐릭터 무리가 먹히는 건 엔진이 나머지를 맡기 때문이다. 그 엔진이 **Glimi Core** 다. 기억은 프롬프트가 아니라 SQLite 등에 쌓인다. 재시작하거나 모델을 Haiku→로컬 Llama 로 바꿔도 관계·사실·기억이 그대로다. 기억은 `num_ctx` 설정에 맞게 자른다. 컨텍스트가 4096(Ollama) 이든 16384 이든 성격이 일정하다. 모델은 캐릭터마다 클라우드(Claude), 로컬(Ollama), Grok CLI 를 섞을 수 있다. 전부 로컬이면 비용은 없다.

엔진 대시보드에서 관계 그래프, 기억 인스펙터, 채널 뷰어, 도구 호출 타임라인, LLM 비용·사용량을 실시간으로 본다.

![Glimi — 살아있는 친구들의 커뮤니티, 커넥션 그래프에서 라이브로](docs/screenshots/en/11-community-dashboard.png)

이 Core 위에 앱이 올라간다. 플래그십은 **Glimi Community** 다. 내장 웹 UI(또는 디스코드)에서 대화하는 'AI 친구들' 무리로, 자기 채널에서 소통하고 비밀을 지키며 당신이 없어도 당신을 이야기하고 기억한다. **Glimi Workspace** 는 역할을 나눈 작업 공간이다. Coordinator 가 Researcher·Builder·Critic 에게 일을 나눈다. 실시간 데모와 `examples/` 스타터도 같은 Core 위에 선다.

> 용어: 여기서 "에이전트" 는 *Generative Agents* 계보의 캐릭터 개념이다. 기억하고 생각하고 서로 말하는 존재이지, task-runner 가 아니다. 코드에서 *agent*, 사용자 쪽에선 *친구·캐릭터*.
...
> **한 레포, 세 프로젝트.** Glimi Core(`glimi-core/`, `glimi` 패키지)는 **Glimi Community(`glimi-community/`)에서 추출**한 커널이다. **Glimi Workspace**(`glimi-workspace/`) 는 Core 위에 새로 세운 앱이다(커뮤니티 import 없음). 두 앱은 Core 의 재사용성을 증명한다. 각 폴더는 독립 `pyproject.toml` 을 가진다. 두 앱은 `glimi[dashboard]` 에 의존하며, 이 레포에서는 로컬 editable, 공개 후엔 PyPI 배포판으로 쓴다. 폴더별로 `cd` 하면 각각 독립 실행된다. `glimi` 만은 단독 PyPI 배포된다.

---

## Glimi 의 차별점

Glimi Core 는 세션이 끊겨도 처음으로 돌아가지 않는 에이전트 엔진이다. 일반 도구는 요청마다 역할을 새로 띄우고 문맥이 차면 압축·복원하지만, Glimi 는 그 과정을 생략한다. 각 에이전트가 자기 맥락과 결정, 사용자의 취향·가치를 저장소에 들고 있어서 모델을 바꿔도 그대로 따른다. 이 영속성이 일에서는 **Glimi Workspace**, 사람 관계에서는 **Glimi Community** 로 나타난다. 하나는 매번 브리핑이 필요 없는 상주 팀, 다른 하나는 기억하는 친구다. 두 앱은 Core 의 예시이며, 엔진은 동일하다.

요즘 오픈소스 에이전트 프레임워크(LangChain/LangGraph, AutoGen, CrewAI, OpenAI Agents SDK, Letta 등)는 대부분 에이전트를 **task** 로 띄우고 버린다. Letta 는 영속 메모리가 있고, Stanford Generative Agents·AI Town 처럼 자율 군집을 연구한 예도 있다. Glimi 는 이런 조각을 **하나의 pip 런타임**으로 묶는다. 핵심은 두 가지다.

**1. 컨텍스트 윈도우 맞춤 메모리 (Elastic Memory).** 메모리 주입을 `num_ctx` 타깃에 맞춰 잘라 프롬프트가 윈도우 안에 머물게 한다. 완전 보장은 아니며 추정치에 마진을 둔 best-effort 방식이다. 그래서 4096 이든 16384 이든 성격이 변하지 않는다. CrewAI·Letta·AutoGen 등도 히스토리를 자르지만, Glimi 처럼 윈도우 크기 자체를 버짓으로 삼는 곳은 없다. Ollama 의 "VRAM 따라 컨텍스트 조절" 이슈는 아직 미해결이다.

**2. 무료·내장 런타임의 드리프트 방지 메모리.** Glimi 의 fact 는 유효기간이 있어 모순된 옛 사실은 supersede 처리된다(이력은 유지). Zep 의 Graphiti 는 UI 포함 독점 엔진이고, Mem0 는 2026 년 모순 해소 기능을 뺐다. Glimi 는 supersession, 런타임, 대시보드를 함께 무료 제공한다. SQLite 행 단위 supersession 으로 작지만 핵심 동작은 같다.

이 통합 위에서 다음이 동작한다.

- **영속 인구.** 각 에이전트의 페르소나와 모델을 정의하고 Claude·Ollama 를 한 fleet 으로 묶는다. 상태가 프롬프트가 아닌 스토리지에 있어 모델을 바꿔도 기억과 관계가 남는다.
- **자율 실행.** proactive supervisor 가 주기적으로 채널을 열고 중단된 대화를 이어간다. 대부분은 reactive 만 지원하나, Glimi 는 인구가 스스로 움직인다.
- **저사양 친화.** 여러 에이전트가 로컬 모델 하나를 공유해 16GB 로도 동작한다. Ollama 상주 모델 위에 상태 관리층을 추가했다.
- **인구 대시보드.** 관계 그래프, L0–L5 메모리 뷰, 라이브 채널, 모델 조회를 실시간으로 보여준다. Letta ADE·Hermes HUD 는 단일 어시스턴트만 보지만 Glimi 는 인구 관계 단위로 본다.

현재 알파(0.1.0, PyPI 미배포) 단계다. 기능 단위로는 Letta·AI Town·SillyTavern·Zep 이 앞서 있지만, Glimi 는 이 조합으로 승부한다.

<!--
### Glimi vs. 대안들

각 프로젝트는 강점이 다르다. Glimi 의 위치는 다음과 같다.
...
✅ 됨 · ◐ 부분 · ❌ 안 됨 · — 해당 없음. 메모리 페이징은 Letta 가 깊고, AI Town 은 완성도가 높고, Zep 의 시간 그래프는 더 완전하며, SillyTavern 은 캐릭터 도구가 많다. Glimi 는 이 일곱 줄을 전부 하나의 AGPL-3.0 패키지로 제공하는 유일한 쪽이다.
-->
...
Glimi 응답은 최대 **8 개 개념 레이어**를 거친다. 일부는 LLM 호출 근처(프롬프트·도구·메모리)에, 나머지는 별도 서브시스템(A2A 루프·supervisor·자가 치유)에 있다. 7개는 reactive, 1개는 proactive(타이머 구동)다.
...
3개(채널 규율, anti-echo, 자가 치유)는 *application 패턴* 중심으로 Community 쪽에 가깝고, 나머지는 Glimi Core 담당이다.

**1 · 프롬프트 조립** — 언어 × agent_type dispatch (`ko/` 가 `en/` 위에 overlay), provider별 도구 dialect (Claude `<tools>` XML, OpenAI function call), locale snippet (`ㅇㅇ` / `ok`, `카톡` / `Discord`).

**2 · 도구 프로토콜** — `ToolSpec` 레지스트리가 권한·타입 검증, dispatcher 가 핸들러 호출 후 결과를 다음 user prompt 에 주입.

**3 · 메모리 파이프라인** — N 턴마다 Haiku 가 `{summary, facts[], relationships[], emotion, entities, importance}` JSON 추출. 에피소드 rollup, 사실 supersession(Zep 스타일), 친밀도 자동 증분. Budget 기반 주입(~1000 토큰/턴): pinned + relationship + episodic-current + cross-channel + retrieved + facts. Retrieval = `0.4·semantic + 0.3·importance + 0.2·recency_decay + 0.1·relational`.

**4 · 채널 규율** — 프롬프트에 채널 참여자 명시. Role bleed 차단 (비밀 채널에서 오너에게 말 거는 회귀 방지).

**5 · Anti-echo / dedup / reality guard** — 작별 핑퐁 차단, 단답 ack 시 도구 재호출 금지, 60초 내 유사 95% 호출 drop, 허위 행동 차단.

**6 · A2A 대화 루프** — `start_conversation(...)` 으로 에이전트 간 대화 시작. 턴 제한과 closure 감지.

**7 · 자가 치유** (실험, 기본 OFF) — `request_dev_fix` 큐잉 → supervisor 트리아지 → 승인 시 Opus subprocess(`GLIMI_DEV_DISPATCH=1`)가 패치 → 봇 재시작 시 요약 주입.

**8 · Supervisors** ⭐ — 타이머 기준 3개 트리오. 페어 스캐너(DB 점수화)로 새 채널 생성, Chat 감시자(Haiku judge)가 멈춘 채널 깨움, Scene 감시자가 phase 진행. **nudge 는 명령이 아니라 내면 생각으로 주입**된다.
...
이 차이가 캐릭터를 유지하는 에이전트를 만든다. 명령은 메타 텍스트로 새고, 혼잣말은 대사에 녹는다.
...
방어 장치:
- `_validate_fact()` 가 추상 subject(`"새_멤버"`), 일시 object(`"오랜만"`), 중복 self-fact drop.
- `PREDICATE_ALIASES` 가 40+ 변형을 canonical 로 정규화해 동의어 분산 방지.
- 비밀 채널 메모리는 오너 채널 주입 시 disclosure 마커 부착.

### 모델 스왑·프로필 수정에도 맥락이 유지되는 이유

- 상태는 프롬프트가 아닌 외부 저장소에 있다. Haiku → Sonnet → 로컬 Llama 로 바꿔도 관계·fact·pinned 그대로며 새 모델이 그것을 읽는다.
- 프로필 편집 시 `invalidate_cache()` 와 `runtime.refresh_agent()` 을 함께 실행해 즉시 반영 — 반복 질문 회귀 방지.

### Quick Start (라이브러리)

Glimi Core 는 **알파 (0.1.0, PyPI 미배포)**. 소스에서 설치한다. 커널은 인메모리 스토어와 **오프라인 `echo` 백엔드**를 내장한다. 아래 예제는 **의존성·API 키 0**으로 바로 돈다. `echo` 는 모델을 호출하지 않고 하네스 배선을 눈으로 확인하게 해 준다:
...
`Glimi` 가 인메모리 `KernelStore`, `ProfileProvider`/`OwnerContext`, `NullObserver`, 지정 LLM 백엔드를 자동 배선한다. 더 제어하려면 각 구성요소를 직접 불러 써라.
...
자체 DB 를 쓰려면 `KernelStore` 와 필요한 provider/observer 를 구현해 `glimi.runtime.set_store(...)` 로 주입한다. 완성 예시(SQLite + Discord)는 아래에 있다:
...
대시보드는 Glimi Core 에 포함된다. 그래프·메모리 인스펙터·채널 뷰어·도구 로그는 전체 에이전트에 동작한다. **읽기 전용**이며 모델 스왑 *쓰기*는 Community/Workspace 기능이다.
...
- **메모리 인스펙터 (L0–L5)** — Pinned, 에피소드, 의미 사실, 관계 변곡점
- **실시간 채널 뷰어** — 각 에이전트의 실제 시점 표시
- **도구 호출 타임라인** — `<tools>` 호출 이력과 결과
...
1인 회사에도 팀이 있다. Glimi Workspace 에선 Coordinator 와 Researcher·Builder·Critic 이 역할을 나눈다. 프로젝트 맥락은 한 번만 정리하면 된다. 무엇을 하고 왜 그렇게 했는지 정도면 충분하다. 각 에이전트가 이 맥락을 들고 있어 새 세션마다 설명할 필요가 없다. Haiku 에서 Sonnet 으로, 클라우드에서 로컬로 바꿔도 같은 팀이 그대로 이어간다. 도구가 아니라 함께 일하며 맥락을 쌓는 상주 인력에 가깝다.

Workspace 와 Community 는 *같은* Core 위에 다른 목적의 앱이다. 하나는 일하는 팀, 다른 하나는 당신을 기억하는 친구들이다. Core 가 모놀리식이 아님을 보여준다. Workspace 는 `glimi` 패키지만 import 한다(디스코드 0, Community 코드 0).

팀은 실제처럼 상호작용한다. 오너가 Coordinator 에게 DM 을 보내면, Coordinator 가 일을 나누고 전문가들이 에이전트-투-에이전트 채널에서 토론한다. 결과가 그룹 라운드에서 모이면 Coordinator 가 전달한다. 이 기록이 Community 연결 그래프의 엣지가 된다. 팀은 상호작용망으로, 멤버는 각자 L0–L5 메모리를 가진다.
#### 한 서버에 여러 워크스페이스

`./run.sh workspace` 를 실행하면 서버 하나가 **여러 워크스페이스**를 띄운다(Community 가 여러 커뮤니티를 다루듯). 읽기 전용 **데모 워크스페이스**가 포함되어 있고, 이름과 목표만 주면 새 워크스페이스를 만들 수 있다. 그 순간 새 팀이 꾸려진다. 워크스페이스를 열면 그 팀의 작업을 즉시 볼 수 있다.
<img src="docs/screenshots/en/15-workspace-home.png" alt="Glimi Workspace — 한 서버에 여러 워크스페이스" width="820"/>

#### 라이브로 보기

데모 워크스페이스는 저장된 팀을 계속 돌려 보여주는 실시간 쇼케이스다. 오프라인·API 키 불필요·**$0**. 한 화면에 그래프, 멤버 메모리·fact, 채널 뷰어(DM, A2A 토론, 그룹 라운드, `mgr-approvals` HITL), 관찰성 패널이 뜬다. 도구 호출 타임라인과 LLM 사용량 카드(로컬/echo 는 $0, *est.* 표시)가 함께 보인다.
...
Coordinator 가 결과를 커밋하기 전 Workspace 는 **승인 게이트**를 거칠 수 있다. 오너가 승인·수정·거부 중 선택하고, 거부 시 폴백이 실행된다. 기록은 `mgr-approvals`(대시보드에서 확인)에 남는다. 설정(`--approve auto|final|off`)에 따라 자동·수동 승인되고 흐름은 멈추지 않는다. CI·파이프·데모 실행은 자동 승인된다. 핵심 액션마다 HITL 체크포인트가 있어 사후 확인이 가능하다.
---

## EDD — eval-driven development (커밋마다 추적되는 품질) ⭐

멀티 에이전트 제품은 증명하기 어렵다. "더 진짜 같다"는 건 감상이지 수치가 아니다. Glimi 의 답은 **EDD(eval-driven development)** 다. 자율 **오너 에이전트**가 앱을 온보딩부터 핵심 저니까지 돌리고, 세션을 **가중 차원**으로 채점해 **0–100 종합 점수**로 묶는다. 런마다 **git-SHA 앵커 "세대"** 로 커밋해 `git log` 가 품질 타임라인이 된다. 프레임워크는 **`glimi.edd`** 로, `glimi` 커널 일부이며 Community·Workspace **둘 다 상속**한다(자기 차원 + 오너 에이전트만 구현).

**채점 방식** — 각 차원 0–10 점 + 가중치, 종합은 0–100 정규화. `critical` 실패는 종합과 무관하게 FAIL. LLM-judge 차원은 `echo` 백엔드 없을 때 **skip**(가짜 점수 없음). 무료 셀프테스트가 점수를 높일 수 없다. Community 기본 6차원:
...
여기 **레포에 커밋된 세대들**(`tests/e2e/qa_generations/*.json`)이 있다. `claude_cli` 런을 judge 가 채점한 결과로, 각 git SHA 가 박혀 있다. N 은 작지만 요점은 긴 기록이 아니라 **세대별 데이터 축적 방법론**이다. 읽으면 이미 흐름이 보인다:
...
`*` = 커밋 시점 dirty. 점수는 커밋 JSON 그대로다. **gen-11 이 전환점**. web-native 온보딩 빌드(gens 5–10 이 그쪽으로)로 critical `friend_creation` 을 **0 → 10** 만들며 첫 ✅ PASS(85/100). 하네스가 예고했던 `0 → 10` 점프다.

숫자 공개 이유는 이렇다.
- **`conversation_quality` 6.0 → 9.0 → 8.0 → 4.0 … → 7.0.** LLM 의 변동성을 보여준다. 단일 스크린샷보다 *트렌드*가 필요하다. gen-1→2 는 매니저가 중복 질문을 멈춘 개선을, gen-4 는 회귀를, gen-11 은 7.0 재안정화를 보여준다.
- **`friend_creation` 은 `critical`, gens 1–10 에서 0.0.** 초기 런은 전부 FAIL. 하네스가 **정상 작동**한 셈이다. supervisor 가 디스코드 봇 프로세스 안에만 있어 웹 E2E 가 안 돌던 **어댑터 갭**을 수치로 남겼다. web-native 빌드에서 해결돼 **gen-11 `friend_creation` = 10.0 → 첫 ✅ PASS(85/100)**. 점수는 예고된 대로 `0 → 10`. (`conversation_quality` 7.0, `no_hallucination` 6.0 은 아직 개선 중.)

한 줄 요약: **품질을 git 에 1급 메트릭으로 남긴다.** 모든 커밋 영향이 측정되고 보인다. 아래 대시보드·PDF 가 그 타임라인을 보여준다.
### 보기: `/admin/qa` 대시보드 + PDF 리포트

`/admin/qa` 에 **QA 대시보드**가 있다(admin 로그인 → "QA"). 최신 점수, **품질 트렌드 차트**, 세대별 테이블이 표시된다. 어떤 세대든 **PDF** 로 내보낼 수 있다(`glimi.edd.report` 가 HTML 1페이지를 만들고 → Playwright headless Chromium 으로 출력. 트렌드 라인은 서버 렌더 SVG 로 그대로 인쇄된다).
...
**어댑터 재사용.** `glimi.edd` 는 `glimi` 휠에 포함되고 도메인 중립이다. 차원 + 오너 에이전트만 구현하면 종합 채점, git-앵커 세대 스토어(SQLite + 커밋 JSON), HTML/PDF 리포트를 바로 쓸 수 있다:
```python
...
Community 는 6차원을 쓰고, Glimi Workspace 는 같은 `glimi.edd` 코어를 산출물/위임/A2A 차원에 쓴다. 하나의 EDD 프레임워크, 두 앱이다. 전체 설계는 [`docs/qa_system.md`](docs/qa_system.md)에 있다.
---

## Examples

Community 소셜 sim 없이 Glimi Core 를 직접 실행하는 스타터다. `echo` 백엔드는 의존성·API 키 없이 바로 쓸 수 있다. 실제 모델로 바꾸면 협업이 시작된다.
...
**완료 — 커널 추출 + 패키징**
- ✅ `community/core/{runtime, tools, memory, llm, conversation}` → 최상위 `glimi/` 로 이동. 스토리지·플랫폼 의존 제거, 단독 import 가능.
- ✅ `KernelStore` ABC, `AgentProfile`/`OwnerContext`/`KernelObserver` protocol 구현. Community 는 `community/adapters/` 에서 어댑터 연결.
- ✅ `pyproject` 분리: `pip install glimi`(코어만) / `glimi[community]`(앱). 커널 wheel 빌드 완료.
...
**그다음 — 로컬 모델 백엔드**
- vLLM, llama.cpp 백엔드 추가 예정. (Ollama·Grok 이미 지원, `AVAILABLE_MODELS` 스텁 있음)
- 대시보드에서 에이전트별 로컬 모델 지정.

**그다음 — 에이전트별 RAG 메모리** ⭐
- L0–L5 메모리는 컨텍스트 안에서 동작하지만, 오래된 에이전트는 기억이 윈도우를 넘긴다. 각 에이전트에 RAG 코퍼스를 두어 검증된 retrieval 코어로 기록을 검색한다. 모든 히스토리를 프롬프트에 실지 않고 **관련 항목만 가져온다**.
- **효과**: 큰 히스토리에서도 안정적 회상(`O(top-k)`), 정확한 출처, 드리프트 최소화.
- **지연 표현**: 검색은 인메모리보다 느리다. 에이전트가 온로드 상태에서 RAG 를 호출하며 *"잠시만…"*, *"기억 더듬는 중…"* 같은 말로 랙을 캐릭터 행동처럼 보이게 한다.
...
> 🆕 **처음 기여?** **[`START_HERE.html`](START_HERE.html)** 부터 보세요. 플랫폼 셋업, 첫 task(로컬 모델 지원), Claude Code 워크플로우, 브랜치 전략, 로드맵이 있습니다. **PR 전 반드시 읽기.**

### 첫 contributor task — 로컬 모델 지원 (Gemma 4 / Qwen 3.5)

첫 작업은 Ollama 로컬 LLM 백엔드 구현과 Gemma 4·Qwen 3.5 벤치마크입니다(모델 역할: chat, judge, 메모리 추출 JSON). 현재 Glimi는 Anthropic API에 의존하므로 벤더 중립성을 검증해야 합니다. 자세한 내용은 [`START_HERE.html` §5](START_HERE.html#first-task)에 있습니다.
...
- **메모리 / 감정은 user prompt 동적 주입.** system prompt 고정 금지. `AgentRuntime`이 채널별 턴마다 구성.
- **타임스탬프는 UTC-aware ISO**(`community.core.timeutil.now_utc_iso()`). SQLite `CURRENT_TIMESTAMP` 금지.
- **"에이전트"·"봇"·"AI" 표현 금지.** `<tools>` 블록은 대화 채널 비노출. 도구 호출 로그는 `logs/system.log`.
- **프로필 편집 시** `invalidate_cache()`, `runtime.refresh_agent()` 함께 호출.

### 커밋 규칙

- 제목 1줄(50자 내외), 본문 최대 2줄.
...
**AGPL-3.0-or-later**는 강한 카피레프트다. 누구나 쓸 수 있지만, **배포나 네트워크 서비스 시 파생물 소스 공개와 저자 표기 유지**가 의무다. 독점 제품화는 불가하다. 기여는 동일 라이선스로 받고, 저작권은 저자가 보유해 상업 라이선스를 별도 제공할 수 있다. MongoDB, Grafana, Mastodon처럼 "열린 개발과 free-riding 방지" 방식을 따른다.

자세한 내용은 `LICENSE` 파일을 본다.
