...
> **현황 (2026-06)** — Core 커널은 최상위 `glimi/` 패키지로 추출 완료(Discord/DB 의존 0, 단독 import). 아직 PyPI 전이라 소스에서 설치한다 (`pip install -e .`). 이제 캐릭터와 얘기하는 기본 경로는 내장 **웹 챗**(라이트/다크, 답글, 반응(reactions), 스레드, 모바일)이라 **디스코드는 선택사항**이 됐다 — 예정된 여러 어댑터 중 하나일 뿐. 최근 추가: **평가 하네스**(골든셋 + LLM-as-judge + 회귀 게이트), 대시보드의 **도구 호출 + 비용/지연 관찰성**, 그리고 Workspace 의 **사람 개입 승인 게이트(human-in-the-loop)**.
...
│   └── workspace/                · Glimi Workspace        (역할 기반 작업 팀 + --demo)
├── examples/                   · 라이브러리 스타터
│   ├── research_buddies/       · 두 에이전트가 주제 협업
│   ├── dev_pair/               · planner + executor
│   └── dashboard_demo/         · 임의 인구를 Core 대시보드로 관찰
├── docs/
├── tests/
├── LICENSE                     · Apache-2.0
├── README.md                   · 영문
...
#### 라이브로 보기 — `./run.sh workspace --demo`

시드된 실시간 데모(Community 쇼케이스의 Workspace 짝)가 런치 팀을 저장소에 올리고 백그라운드 루프로 계속 움직여서, 대시보드가 보는 앞에서 갱신된다 — 오프라인, API 키 불필요, **$0**. 한 화면에 전부 보인다: 그래프, 멤버별 메모리·fact, 채널 뷰어(오너 DM, 위임 DM, A2A 토론, 그룹 라운드, 그리고 `mgr-approvals` HITL 기록), 그리고 관찰성 패널 — 도구 호출 타임라인과 정직한 LLM 사용량 카드(로컬/echo 는 $0, 모든 카운트에 *est.* 표시).
...
```bash
./run.sh workspace --demo               # 시드된 라이브 데모 → http://127.0.0.1:8800
